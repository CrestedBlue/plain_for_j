    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-white">내 여행</h2>
            <p className="text-sm text-slate-400 mt-1">여행묶음을 선택하거나 새로 만들어 보세요.</p>
          </div>
          <button
            onClick={onNewTrip}
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 list-none p-0">
          {trips.map((t) => (
            <li
              key={t.id}
              className="relative bg-slate-800/80 rounded-2xl border border-slate-700/50 shadow-lg hover:border-indigo-500/50 hover:-translate-y-1 transition-all duration-300 group"
            >
              ...
              <button
                ...
                className="absolute top-3 right-3 p-2 rounded-lg text-slate-500 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 [@media(hover:none)]:opacity-100 hover:text-rose-400 hover:bg-rose-500/10 transition"
              >
